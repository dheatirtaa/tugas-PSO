...
sinyalpersegi = sinyal_pers(30, 30) #batas atas dan batas bawah dari sinyal persegi
sinyalf = dft(sinyalpersegi)
df2 = [i**2 for i in sinyalf]
flog = [log(i) for i in df2]
df22 = dft(flog)
cst = [i**2 for i in df22]

plt.subplot(1, 2, 1)
plt.title('sinyal real (-A to A)')
plt.stem(sinyalpersegi, 'b', markerfmt='o', basefmt='b')

plt.subplot(1, 2, 2)
plt.title('MFCC')
plt.stem(cst, 'b', markerfmt='o', basefmt='b')
plt.show()